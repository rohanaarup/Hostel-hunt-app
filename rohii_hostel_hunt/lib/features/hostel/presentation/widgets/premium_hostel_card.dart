...
class _PremiumHostelCardState extends State<PremiumHostelCard>
  ...
  void _onWishlistTap() {
    ...
    setState(() => _wishlistBounce = true);
    Future.delayed(const Duration(milliseconds: 200), () {
      if (mounted) setState(() => _wishlistBounce = false);
    });
      ...
      SnackBar(
        content: Row(
          children: [
            ...
            Flexible(
              child: Text(
                '${widget.hostel.name} added to wishlist ❤️',
  ...
  Widget _buildImage(Hostel hostel) {
    ...
    if (imagePath.isEmpty) return errorWidget;

    // Local asset
    if (hostel.isLocal || (!imagePath.startsWith('http') && !imagePath.startsWith('/'))) {
      return Image.asset(
        imagePath,
        fit: BoxFit.cover,
    ...
    // Network image — prepend base URL for relative paths
