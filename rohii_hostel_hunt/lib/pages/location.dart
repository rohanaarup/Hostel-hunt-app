...
import 'package:rohii_hostel_hunt/services/location_model.dart';
...
class _LocationScreenState extends State<LocationScreen>
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      if (mounted) {
        context.read<LocationProvider>().detectCurrentLocation();
      }
    });
  }
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      _animated(1, _CurrentLocationCard(
                        ...
                        onTap: () => _handleCurrentLocation(locProvider),
                      )),
                      ...
                      _animated(4, _SavedAddressesSection(
                        isDark: isDark,
                        addresses: locProvider.savedAddresses,
                        onSelect: (addr) {
                          locProvider.selectAddress(addr);
                          Navigator.pop(context);
                        },
                        onDelete: (id) => locProvider.deleteAddress(id),
                      )),
                    ],
                  ),
                );
  ...
  void _handleCurrentLocation(LocationProvider provider) {
    HapticFeedback.lightImpact();
    if (provider.isDetectingLocation) return;

    if (provider.currentLocationText.isNotEmpty &&
        provider.locationError == null) {
      // Use the detected location
      provider.setCity(
        provider.currentLocationText.split(',').last.trim(),
      );
      Navigator.pop(context);
    } else {
      // Retry
      provider.detectCurrentLocation();
    }
  }
  ...
  Widget _buildContent() {
    return Column(
      ...
      children: [
        ...
        // State: resolved
        else if (provider.currentLocationText.isNotEmpty)
          Text(
            provider.currentLocationText,
            style: TextStyle(
